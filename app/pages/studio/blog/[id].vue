...
const viewTranslation = computed(() => {
  ...
  return post.value.translations.find((t: { locale: string }) => t.locale === currentLocale.value) 
      || post.value.translations.find((t: { locale: string }) => t.locale === 'en')
        ...
        <UiCardContainer class="flex flex-col justify-between gap-6 px-8 py-6 lg:flex-row lg:items-center lg:gap-8">
            <div class="flex w-full items-center gap-5 lg:w-auto">
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <span class="rounded-lg border px-3 py-0.5 text-xs font-bold capitalize shadow-sm" :class="getStatusColor(post.status)">
                            {{ post.status }}
                        </span>
                    </div>
                </div>
    ...
    <div class="grid grid-cols-1 items-start gap-12 lg:grid-cols-4">
        ...
        <div class="space-y-12 lg:col-span-3">
            ...
            <div v-if="!isEditing && post && viewTranslation" class="flex flex-col items-start">
                ...
                <div class="prose prose-lg prose-neutral w-full max-w-none dark:prose-invert prose-headings:font-bold prose-headings:tracking-tight prose-a:text-secondary-500 prose-a:no-underline hover:prose-a:underline prose-img:rounded-3xl prose-img:shadow-2xl" v-html="renderedBody"></div>
            </div>
                ...
                <UiCard>
                    <UiCardContainer class="space-y-6 p-8">
                        ...
                        <div class="border-t border-neutral-100 pt-4 dark:border-neutral-800">
                            ...
                            <UiInput id="post-image-alt" v-model="editablePost.common.coverImageAlt" label="Alt Text" class="mt-4" />
                        </div>
                    </UiCardContainer>
                </UiCard>
            ...
            <UiCard v-if="!isEditing && post && viewTranslation" class="p-6">
                ...
                <div class="space-y-4 text-sm">
                    <div class="flex flex-col gap-1">
                        ...
                        <div class="flex">
                            <span class="rounded border px-2 py-0.5 text-[10px] font-bold uppercase capitalize shadow-sm" :class="getStatusColor(post.status)">
                                {{ post.status }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <span class="rounded bg-neutral-100 px-1 font-mono text-xs dark:bg-neutral-800">{{ post.translationKey }}</span>
                    </div>
                </div>
            </UiCard>
            ...
            <template v-else-if="isEditing && editablePost">
                <UiCard>
                    <UiCardContainer class="space-y-6 p-6">
                        ...
                        <UiSelect id="post-status" v-model="editablePost.common.status" label="Status" :options="['draft', 'published', 'archived']" />
                        <UiInput id="post-date" v-model="editablePost.common.publishedAt" label="Publish Date" type="datetime-local" />
                        
                        <UiInput id="post-category" v-model="editablePost.common.categoryName" label="Category" />
                        ...
                        <div>
                            ...
                            <div class="mt-2 flex flex-wrap gap-2">
                                <UiChip v-for="tag in editablePost.common.tags" :key="tag" size="sm">{{ tag }}</UiChip>
                            </div>
                        </div>
                    </UiCardContainer>
                </UiCard>
