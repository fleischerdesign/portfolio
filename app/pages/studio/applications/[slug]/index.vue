...
const bodyStats = computed(() => {
  const text = isEditing.value
    ? editableApplication.value?.body
    : application.value?.body;
...
<template>
  ...
  >
      ...
      <div class="space-y-12 lg:col-span-3">
            ...
            <div class="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-3">
              <div class="space-y-8">
                ...
                <div class="space-y-6">
                  <UiInput
                    id="edit-title"
                    v-model="editableApplication.title"
                  ...
                  <UiInput
                    id="edit-subtitle"
                    v-model="editableApplication.subtitle"
                    :label="$t('applications.detail.config.subtitle_label')"
                    class="dashboard-input"
                  />
                  <UiInput
                    id="edit-url"
                    v-model="editableApplication.url"
              ...
              >
                ...
                <div class="space-y-6">
                  ...
                  <div
                    v-if="editableApplication.selectedCompany?.address"
                    ...
                    <div class="flex items-start justify-between">
                      <div class="text-sm">
                        <p class="font-bold text-neutral-900 dark:text-white">
                          {{ editableApplication.selectedCompany.name }}
                        </p>
                        <p class="mt-1 text-neutral-500">
                          {{
                            editableApplication.selectedCompany.address.street
                          }}
                          {{
                            editableApplication.selectedCompany.address
                              .houseNumber
                          }}
                        </p>
                        <p class="text-neutral-500">
                          {{
                            editableApplication.selectedCompany.address.zipcode
                          }}
                          {{ editableApplication.selectedCompany.address.city }}
                        </p>
                      </div>
                      <UiButton
                        size="sm"
                        ...
                        @click="showCompanyAddressModal = true"
                      >
                ...
                >
                  <template #display="{ option }">
                    ...
                    <span v-if="option.companyId" class="ml-1 opacity-50"
                      >({{
                        allCompanies.find((c) => c.id === option.companyId)
                          ?.name
                      }})</span
                    >
                  </template>
        ...
        >
          <UiCard
            ...
            :key="contact.id"
            class="border-secondary-500/5"
          >
            <UiCardContainer class="!flex-row items-center gap-4">
              <div
                class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-secondary-100 text-secondary-600 dark:bg-secondary-900/30 dark:text-secondary-400"
              >
              ...
              <div>
                ...
                <p class="text-xs text-neutral-500 dark:text-neutral-400">
                  {{ contact.position || "Ansprechpartner" }}
                </p>
                <div
                  class="mt-1 flex gap-3 text-xs text-secondary-600 dark:text-secondary-400"
                >
                  ...
                  <span v-if="contact.phone" class="flex items-center gap-1"
                    ><Icon name="heroicons:phone" /> {{ contact.phone }}</span
                  >
                </div>
              </div>
            </UiCardContainer>
        ...
        <div
          v-if="
            (!isEditing && application.body) ||
            (isEditing && editableApplication)
          "
            ...
            <UiCardContainer class="p-8 md:p-16 lg:p-24">
              ...
              >
                ...
                <div class="flex-1">
                  <div class="flex items-center justify-between">
                    <h3
                      class="text-3xl font-black text-neutral-900 dark:text-white"
                    >
              ...
              <div class="mb-12">
                ...
                >
                  <template
                    v-if="
                      isEditing && editableApplication?.selectedCompany?.address
                    "
                  >
                    <p>
                      {{ editableApplication.selectedCompany.address.street }}
                      {{
                        editableApplication.selectedCompany.address.houseNumber
                      }}
                    </p>
                    <p>
                      {{ editableApplication.selectedCompany.address.zipcode }}
              ...
              <p
                class="mb-8 text-lg font-bold text-neutral-900 dark:text-white"
              >
                {{ salutation }},
              </p>
                  ...
                  <div class="flex items-center gap-8">
                    ...
                    <div class="hidden flex-col sm:flex">
                      <span
                        class="text-[9px] font-black uppercase tracking-widest text-neutral-400"
                        >{{
                          $t("applications.detail.editor.reading_time")
                        }}</span
                      >
                      <span
          ...
          <UiCardContainer class="flex w-full flex-col gap-3">
            ...
            <template v-if="application.pdfGeneratedAt">
              <UiButton
                variant="glass"
                class="w-full"
                :to="`/api/applications/${slug}/pdf/download`"
                external
              >
    ...
    <UiModal v-model="showAddHistoryModal">
      ...
      <template #footer>
        <UiButton variant="secondary" @click="showAddHistoryModal = false">{{
          $t("applications.detail.actions.cancel")
        }}</UiButton>
        <UiButton @click="addHistory">{{
          $t("applications.detail.actions.save")
        }}</UiButton>
      </template>
    </UiModal>
    ...
    <UiModal v-if="editableHistoryEntry" v-model="showEditHistoryModal">
      ...
      <template #footer>
        <UiButton variant="secondary" @click="showEditHistoryModal = false">{{
          $t("applications.detail.actions.cancel")
        }}</UiButton>
        <UiButton @click="updateHistory">{{
          $t("applications.detail.actions.save")
        }}</UiButton>
      </template>
    </UiModal>
    ...
    <UiModal v-if="deletableHistoryEntry" v-model="showDeleteHistoryModal">
      ...
      <template #footer>
        <UiButton variant="secondary" @click="showDeleteHistoryModal = false">{{
          $t("applications.detail.actions.cancel")
        }}</UiButton>
        <UiButton color="danger" @click="deleteHistory">{{
          $t("applications.card.delete")
        }}</UiButton>
      </template>
    </UiModal>
    ...
    <UiModal
      v-if="editableApplication?.selectedCompany"
      v-model="showCompanyAddressModal"
    >
      <template #header
        ><h3 class="text-2xl font-black">
