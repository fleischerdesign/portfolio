...
const viewTranslation = computed(() => {
  ...
  return project.value.translations.find((t: { locale: string }) => t.locale === currentLocale.value) 
      || project.value.translations.find((t: { locale: string }) => t.locale === 'en')
  ...
  <div class="container mx-auto max-w-screen-xl px-4 pb-16 pt-32 md:px-8 lg:pt-44">
    ...
    <div class="mb-12 flex flex-col gap-8">
        ...
        <UiSectionHeader 
            ...
            :level="1" 
            :title="(editableProject as any)[currentLocale].title || 'Editing Project'" 
    ...
    <div class="grid grid-cols-1 items-start gap-12 lg:grid-cols-4">
            ...
            <div v-if="!isEditing && project && viewTranslation" class="space-y-12">
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                    <UiCard v-if="viewTranslation.features?.length" class="p-6">
                        ...
                        <ul class="list-disc space-y-1 pl-4 text-sm text-neutral-600 dark:text-neutral-400">
            ...
            <div v-else-if="isEditing && editableProject" class="space-y-8">
                <UiCard>
                    <UiCardContainer class="space-y-6 p-8">
                        <UiInput id="project-title" v-model="(editableProject as any)[currentLocale].title" label="Title" class="text-xl font-bold" />
                        <UiInput id="project-slug" v-model="(editableProject as any)[currentLocale].slug" label="Slug" class="font-mono text-sm" />
                        <UiInput id="project-subtitle" v-model="(editableProject as any)[currentLocale].subtitle" label="Subtitle" as="textarea" rows="2" />
                        ...
                        <div class="border-t border-neutral-100 pt-4 dark:border-neutral-800">
                            ...
                            <UiInput id="project-image-alt" v-model="(editableProject as any).common.coverImageAlt" label="Alt Text" class="mt-4" />
                        </div>
                    </UiCardContainer>
                </UiCard>
                ...
                <div class="grid h-[600px] grid-cols-1 gap-4 md:grid-cols-2">
                    <UiCard class="flex h-full flex-col overflow-hidden">
                        <div class="border-b border-neutral-200 bg-neutral-50 px-4 py-2 text-xs font-bold uppercase tracking-widest text-neutral-500 dark:border-neutral-800 dark:bg-neutral-900/50">Markdown</div>
                        <textarea 
                 ...
                 <UiCard>
                    <UiCardContainer class="space-y-6 p-6">
                        ...
                        <div>
                             <label class="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">Features (one per line)</label>
                             <textarea 
                                :value="(editableProject as any)[currentLocale].features.join('\n')"
                                class="w-full rounded-lg border-neutral-300 bg-transparent text-sm dark:border-neutral-700"
                                rows="5"
                                @input="e => (editableProject as any)[currentLocale].features = (e.target as HTMLTextAreaElement).value.split('\n').filter(Boolean)"
                             ></textarea>
                        </div>
                         <div>
                             <label class="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">Learned (one per line)</label>
                             <textarea 
                                :value="(editableProject as any)[currentLocale].learned.join('\n')"
                                class="w-full rounded-lg border-neutral-300 bg-transparent text-sm dark:border-neutral-700"
                                rows="5"
                                @input="e => (editableProject as any)[currentLocale].learned = (e.target as HTMLTextAreaElement).value.split('\n').filter(Boolean)"
                             ></textarea>
                        </div>
                         <div>
                             <label class="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">Challenges (one per line)</label>
                             <textarea 
                                :value="(editableProject as any)[currentLocale].challenges.join('\n')"
                                class="w-full rounded-lg border-neutral-300 bg-transparent text-sm dark:border-neutral-700"
                                rows="5"
                                @input="e => (editableProject as any)[currentLocale].challenges = (e.target as HTMLTextAreaElement).value.split('\n').filter(Boolean)"
                             ></textarea>
                        </div>
                    </UiCardContainer>
                 </UiCard>
        ...
        <aside class="sticky top-24 space-y-6">
            ...
            <UiCard v-if="!isEditing && project && viewTranslation" class="p-6">
                 <h3 class="mb-4 text-xs font-bold uppercase tracking-widest text-secondary-500">Meta Data</h3>
                 <div class="space-y-4 text-sm">
                      <div class="flex flex-col gap-1">
                        <span class="text-xs text-neutral-500">Status</span>
                        <div class="flex">
                            <span class="rounded border px-2 py-0.5 text-[10px] font-bold uppercase capitalize shadow-sm" :class="getStatusColor(project.status)">
                                {{ project.status }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <a v-if="project.repoUrl" :href="project.repoUrl" target="_blank" class="truncate text-secondary-500 hover:underline">{{ project.repoUrl }}</a>
                        <span v-else class="text-neutral-400">-</span>
                    </div>
                     <div class="flex flex-col gap-1">
                        <span class="text-xs text-neutral-500">Project URL</span>
                        <a v-if="project.projectUrl" :href="project.projectUrl" target="_blank" class="truncate text-secondary-500 hover:underline">{{ project.projectUrl }}</a>
                        <span v-else class="text-neutral-400">-</span>
                    </div>
                 </div>
            </UiCard>
            ...
            <template v-else-if="isEditing && editableProject">
                <UiCard>
                    <UiCardContainer class="space-y-6 p-6">
                        ...
                        <UiSelect id="project-status" v-model="(editableProject as any).common.status" label="Status" :options="['draft', 'published', 'archived']" />
                        <UiInput id="project-date" v-model="(editableProject as any).common.publishedAt" label="Publish Date" type="datetime-local" />
                        
                        <UiInput id="project-category" v-model="(editableProject as any).common.categoryName" label="Category" />
                        
                        <div>
                            <label class="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">Tags</label>
                            <input 
                                :value="(editableProject as any).common.tags.join(', ')"
                                class="w-full rounded-lg border-neutral-300 bg-transparent text-sm dark:border-neutral-700"
                                placeholder="vue, nuxt"
                                @input="e => (editableProject as any).common.tags = (e.target as HTMLInputElement).value.split(',').map(t => t.trim()).filter(Boolean)"
                            />
                             <div class="mt-2 flex flex-wrap gap-2">
                                <UiChip v-for="tag in (editableProject as any).common.tags" :key="tag" size="sm">{{ tag }}</UiChip>
                            </div>
                        </div>
                        
                         <div>
                            <label class="mb-1 block text-sm font-medium text-neutral-700 dark:text-neutral-300">Techstack</label>
                            <input 
                                :value="editableProject.common.techstack.join(', ')"
                                class="w-full rounded-lg border-neutral-300 bg-transparent text-sm dark:border-neutral-700"
                                placeholder="typescript, tailwind"
                                @input="e => (editableProject as any).common.techstack = (e.target as HTMLInputElement).value.split(',').map(t => t.trim()).filter(Boolean)"
                            />
                             <div class="mt-2 flex flex-wrap gap-2">
                                <UiChip v-for="tech in (editableProject as any).common.techstack" :key="tech" size="sm">{{ tech }}</UiChip>
                            </div>
                        </div>
                    </UiCardContainer>
                </UiCard>
                ...
                <UiCard>
                    <UiCardContainer class="space-y-6 p-6">
                        ...
                        <UiInput id="project-icon" v-model="(editableProject as any).common.icon" label="Icon (e.g. mage:folder)" />
                         <div v-if="(editableProject as any).common.icon" class="flex items-center gap-2 rounded bg-neutral-100 p-2 dark:bg-neutral-800">
                            <Icon :name="(editableProject as any).common.icon" class="h-6 w-6" />
                            <span class="text-xs">{{ (editableProject as any).common.icon }}</span>
                        </div>

                        <UiInput id="project-repo" v-model="(editableProject as any).common.repoUrl" label="Repo URL" />
                        <UiInput id="project-url" v-model="(editableProject as any).common.projectUrl" label="Project URL" />
                    </UiCardContainer>
                </UiCard>
            </template>
