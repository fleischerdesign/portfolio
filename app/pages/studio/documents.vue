...
const { t } = useI18n();
const { data: documents, refresh, pending } = await useFetch<DocumentPayload[]>('/api/documents');
...
async function handleFileUpload(event: Event) {
  ...
  if (target.files && target.files.length > 0) {
    ...
    if (!uploadForm.value.name) {
      uploadForm.value.name = target.files[0].name.replace(/\.[^/.]+$/, "");
    }
  }
}
  ...
  <div class="container mx-auto max-w-screen-xl px-4 pb-16 pt-32 md:px-8 lg:pt-44">
    ...
    <UiCard class="border-secondary-500/5 shadow-xl shadow-secondary-500/5">
      <UiCardContainer class="p-0">
        <div v-if="pending && !documents" class="p-12 text-center text-neutral-500">
          <Icon name="svg-spinners:ring-resize" class="mb-4 h-8 w-8" />
          <p>Lade Dokumente...</p>
        </div>

        <div v-else-if="documents && documents.length > 0" class="overflow-x-auto">
              ...
              <tr v-for="(doc, index) in documents" :key="doc.id" class="group hover:bg-neutral-50/50 dark:hover:bg-neutral-900/30">
                ...
                <td class="px-8 py-5">
                  <div class="flex items-center gap-4">
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-secondary-100 text-secondary-600 dark:bg-secondary-900/30 dark:text-secondary-400">
                      <Icon :name="doc.fileType.includes('pdf') ? 'heroicons:document-text' : 'heroicons:photo'" size="20" />
                    </div>
                    <div>
                      <p class="font-bold text-neutral-900 dark:text-white">{{ doc.name }}</p>
                      <p class="text-xs text-neutral-400">{{ doc.filename }}</p>
                    </div>
                  </div>
                </td>
                <td class="px-8 py-5 text-sm text-neutral-500">
                  <span class="rounded-md border border-neutral-200 bg-neutral-100 px-2 py-0.5 text-[10px] font-bold uppercase dark:border-neutral-700 dark:bg-neutral-800">
                ...
                <td class="px-8 py-5 text-center">
                  <button 
                    ...
                    :class="doc.isDefault ? 'text-secondary-500' : 'text-neutral-300 hover:text-neutral-400'"
                    @click="toggleDefault(doc)"
                  >
                    <Icon :name="doc.isDefault ? 'heroicons:check-circle-solid' : 'heroicons:no-symbol'" size="24" />
                  </button>
                </td>
                <td class="px-8 py-5 text-right">
                  <div class="flex justify-end gap-2 opacity-0 transition-opacity group-hover:opacity-100">
                    <UiButton size="sm" variant="ghost" :to="`/media/documents/${doc.filename}`" target="_blank">
                      <Icon name="heroicons:eye" class="h-4 w-4" />
                    </UiButton>
                    <UiButton size="sm" variant="ghost" @click="startEdit(doc)">
                      <Icon name="heroicons:pencil" class="h-4 w-4" />
                    </UiButton>
                    <UiButton size="sm" variant="ghost" color="danger" @click="deleteDocument(doc.id!)">
        ...
        <div v-else class="p-20 text-center">
          <div class="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-3xl bg-neutral-50 text-neutral-200 dark:bg-neutral-900">
            <Icon name="heroicons:document-plus" size="40" />
          </div>
          <h3 class="text-xl font-bold text-neutral-900 dark:text-white">Keine Dokumente gefunden</h3>
          <p class="mt-2 text-neutral-500">Lade dein erstes Zertifikat oder Zeugnis hoch.</p>
          <UiButton variant="link" class="mt-4" @click="showUploadModal = true">Dokument jetzt hochladen</UiButton>
        </div>
      </UiCardContainer>
    </UiCard>
    ...
    <UiModal v-model="showUploadModal">
      <template #header><h3 class="text-2xl font-black">Neues Dokument hochladen</h3></template>
      <template #body>
        <div class="flex flex-col gap-6">
          ...
          <div class="space-y-2">
            <label class="text-sm font-bold text-neutral-700 dark:text-neutral-300">Datei (PDF)</label>
            <div class="relative flex min-h-[100px] cursor-pointer items-center justify-center rounded-2xl border-2 border-dashed border-neutral-200 bg-neutral-50/50 p-6 transition-colors hover:border-secondary-500/50 hover:bg-white dark:border-neutral-800 dark:bg-neutral-900/50 dark:hover:bg-neutral-900">
              <input 
                type="file" 
              ...
              />
              <div v-if="uploadForm.file" class="flex items-center gap-3 text-secondary-600">
              ...
              <div v-else class="text-center text-neutral-400">
                ...
                <p class="text-xs font-medium">Klicke zum Auswählen oder Drag & Drop</p>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-3">
             ...
             >
             <label for="is-default" class="text-sm font-medium text-neutral-600 dark:text-neutral-400">
      ...
      <template #footer>
        <UiButton variant="ghost" @click="showUploadModal = false">Abbrechen</UiButton>
        <UiButton :is-loading="isUploading" :disabled="!uploadForm.file || !uploadForm.name" @click="uploadDocument">
          Hochladen
        </UiButton>
      </template>
    </UiModal>
    ...
    <UiModal v-if="editForm" v-model="showEditModal">
      <template #header><h3 class="text-2xl font-black">Dokument bearbeiten</h3></template>
      <template #body>
        <div class="flex flex-col gap-6">
          <UiInput 
            id="edit-doc-name" 
          ...
          <div class="flex items-center gap-3">
             <input 
               ...
               class="h-5 w-5 rounded border-neutral-300 text-secondary-600 focus:ring-secondary-500"
             >
             <label for="edit-is-default" class="text-sm font-medium text-neutral-600 dark:text-neutral-400">
               Als Standard-Anhang für alle Bewerbungen verwenden
             </label>
          </div>

          <div class="rounded-xl border border-neutral-100 bg-neutral-50 p-4 dark:border-neutral-800 dark:bg-neutral-900">
            <p class="text-[10px] font-black uppercase tracking-widest text-neutral-400 mb-1">Datei-Info</p>
            <p class="text-sm font-bold text-neutral-700 dark:text-neutral-300 truncate">{{ editForm.filename }}</p>
            <p class="text-xs text-neutral-400">{{ formatSize(editForm.fileSize) }} • {{ editForm.fileType }}</p>
          </div>
        </div>
      </template>
      <template #footer>
        <UiButton variant="ghost" @click="showEditModal = false">Abbrechen</UiButton>
