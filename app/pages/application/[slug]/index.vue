...
const { data: application } = await useAsyncData(`application-webview-${slug}`, () =>
      ...
      <div class="space-y-8">
        <UiCard class="p-8">
          <div class="space-y-4">
            ...
            <div v-if="application.dates?.application" class="flex items-center gap-2">
              <span class="w-24 font-medium text-neutral-600 dark:text-neutral-400">Beworben am:</span>
              <span>{{ formatDate(application.dates.application) }}</span>
            </div>
            <div v-if="application.dates?.response" class="flex items-center gap-2">
              <span class="w-24 font-medium text-neutral-600 dark:text-neutral-400">Antwort am:</span>
              <span>{{ formatDate(application.dates.response) }}</span>
            </div>
            <div v-if="application.dates?.interviews?.length" class="flex flex-col gap-2">
              <span class="font-medium text-neutral-600 dark:text-neutral-400">Interviews:</span>
              <ul class="list-inside list-disc pl-4">
                <li v-for="(interviewDate, index) in application.dates.interviews" :key="index">
        ...
        <UiCard class="p-8">
                  <article class="prose prose-gray dark:prose-invert max-w-none dark:text-white">
      ...
      <div class="h-full min-h-[80vh]">
        ...
        <div class="mt-4 text-center">
          <a
            :href="printUrl"
            target="_blank"
            class="text-sm text-gray-500 hover:text-primary-500 dark:text-gray-400"
          >
