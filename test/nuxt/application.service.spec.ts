...
describe("ApplicationService", () => {
  ...
  it("should create an application, company and address automatically", async () => {
    const appData: ApplicationCreatePayload = {
      companyId: 0, // Will be overwritten by hook via companyName
    ...
    const result = await applicationService.createOrUpdate(appData);
    ...
    expect(createdApp?.histories[0]!.status).toBe("draft");
  });
  ...
  it("should reuse existing company and update address if needed", async () => {
    ...
    await db
      .insert(companies)
      .values({ name: "Existing Corp", addressId: address!.id });
